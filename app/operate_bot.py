...
필요한 만큼 자료를 찾아 읽고, 정보가 부족하면 되묻고, 후속 업무를 노션 운영 DB에 등록한다.
Drive와 시트는 읽기만 하며 고치지 않는다.
"""
...
def _build_system_prompt(workspace_scoped: bool = False) -> str:
    ...
    prompt = (
        ...
        "어떤 폴더인지, 어떤 문서인지 등이 불분명하면 "
        "질문하고 답을 기다립니다.\n"
        "- 사소한 판단(파일 이름, 문단 순서 등)은 직접 결정하고 무엇을 정했는지만 알려주세요.\n"
        "\n"
        "## 자료는 읽기만 합니다\n"
        "- Drive 파일과 스프레드시트를 고치거나 새로 만드는 기능은 없습니다. "
        "수정·생성 요청을 받으면 할 수 없다고 알리세요.\n"
        "- 정리한 내용을 남겨야 하면 슬랙 답변으로 전달하거나, "
        "운영 업무로 등록해 두는 방법을 제안하세요.\n"
    ...
    if workspace_scoped:
        prompt += (
            "\n"
            "## Drive 작업 공간\n"
            "- Drive 작업 공간이 정해져 있습니다. 검색과 읽기가 그 폴더와 "
            "하위로 제한되어 있습니다.\n"
...
async def answer_operate(
    ...
    """
    # 작업 공간. 지정되면 검색과 읽기가 이 폴더 하위로 제한된다.
