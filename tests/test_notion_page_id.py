...
def test_description_surfaces_in_function_tool_schema():
...
def test_description_surfaces_in_basemodel_tool_schema():
    """pydantic BaseModel args_schema 경로에서도 page_id 설명이 노출된다.

    bare 문자열을 Annotated 에 담으면 BaseModel 경로에서 설명이 무시되어
    update_notion_task_status / create_notion_follow_up_task 의 LLM 가이드가
    사라진다. Field 로 담아야 두 경로 모두에서 노출된다. (회귀 방지)
    """

    class _Model(BaseModel):
        page_id: NotionPageId

    description = _Model.model_json_schema()["properties"]["page_id"]["description"]
    assert "Slack 유저 ID" in description
