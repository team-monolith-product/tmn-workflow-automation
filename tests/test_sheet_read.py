...
def test_유령_열에_옛_응답이_남아_있어도_살아_있는_열을_고른다():
    # **유령 열이 비어 있다고 단정하면 안 된다.** 폼에서 문항을 지우기 전에 들어온
    # 응답은 옛 열에 남는다. 대신 두 열은 행 방향으로 갈린다 -- 한 행에 둘 다
    # 차는 일이 없다. 이것을 "둘 다 값이 있으니 모호" 로 보면 가장 흔한 시트가
    # 통째로 하드 실패한다.
    values = [
        ["타임스탬프", "성함"] + ["성함"],
        ["8/1", "옛응답", ""],
    ...
    assert header == ["성함"]
    # 살아 있는(뒤엣) 열을 골랐다. 첫 행이 빈 것은 옛 응답 행이라 그렇고,
    # 행 자체는 안 버린다 -- 아래 test_행은_통째로_빌_때만_버린다 참고.
    assert [row[0] for row in rows] == ["", "이진선", "김지수"]
