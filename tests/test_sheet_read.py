...
def test_양쪽_다_비면_빈_값이_나온다():
    # 합칠 것이 없으면 빈 칸이다. 행 자체는 통째로 비어 버려진다.
    values = [["성함", "성함"], ["", ""]]
...
def test_유령_열에_옛_응답이_남아_있으면_합친다():
    # **유령 열이 비어 있다고 단정하면 안 된다.** 폼에서 문항을 지우기 전에 들어온
    # 응답은 옛 열에 남는다. 대신 두 열은 행 방향으로 갈리므로 한 행에 둘 다
    # 차는 일이 없고, 그래서 **고르지 않고 합칠 수 있다** -- 아무도 안 잃는다.
    values = [
        ["타임스탬프", "성함", "성함"],
        ["8/1", "옛응답", ""],
        ["8/2", "", "이진선"],
...
def test_정렬해도_같은_답이_나온다():
    # 어느 쪽이 살아 있는지를 순서로 추측하면(뒤엣것·마지막에 찬 것) 이름순으로
    # 정렬해 둔 시트에서 죽은 열을 골라 산 사람이 빈칸이 된다. 예외도 안 난다.
    rows = [
        ["8/1", "옛응답", ""],
        ["8/2", "", "이진선"],
        ["8/3", "", "김지수"],
    ...
    _, 시간순 = read.pick([head] + rows, ["성함"])
    _, 정렬됨 = read.pick([head] + rows[1:] + rows[:1], ["성함"])

    assert sorted(row[0] for row in 시간순) == sorted(row[0] for row in 정렬됨)
    assert sorted(row[0] for row in 정렬됨) == ["김지수", "옛응답", "이진선"]


def test_한_행에_둘_다_차_있으면_고르지_않는다():
    # 행이 겹치면 폼 유령이 아니라 서로 다른 열이다. 조용히 하나를 고르면
    # 절반의 사람이 명단에서 빠진다.
    values = [["성함", "성함"], ["가", "나"], ["다", "라"]]

    with pytest.raises(read.AmbiguousColumn, match="한 행에"):
        read.pick(values, ["성함"])
...
def test_행은_통째로_빌_때만_버린다():
    # 고른 칸이 비었다고 버리면 같은 탭인데 어느 열을 골랐느냐로 행 수가 갈린다.
    # 실측(8/26): 297행 시트가 성함만 고르면 197행, 발송여부를 끼면 297행.
    # 그러면 두 번 나눠 읽어 짝지을 때 다른 사람끼리 붙는다.
    values = [
        ["성함", "발송여부"],
        ["가", "O"],
        ["", "O"],  # 누가 발송여부만 끌어내린 행
    ...
    for cols in (["성함"], ["발송여부"], ["성함", "발송여부"], []):
        _, rows = read.pick(values, cols)
        assert len(rows) == 2, f"{cols} 에서 행 수가 갈렸다"
