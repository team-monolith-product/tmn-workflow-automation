...
블록)로 의존성과 새 버전을 읽고, 이전 버전만 본문에서 보강하여 버전 변경 폭을
판별합니다. head 커밋에 존재하는 모든 체크가 통과했는지 확인하여 approve + 병합합니다.
...
def find_previous_version(message: str, name: str, new_version: str) -> str | None:
    """
    커밋 메시지에서 특정 의존성의 변경 전 버전을 찾는 함수

    YAML 블록은 새 버전(dependency-version)은 주지만 이전 버전은 주지 않으므로,
    본문/제목의 "... from <prev> to <new_version>" 줄에서 prev만 추출합니다.
    알려진 새 버전을 앵커로 삼아 그룹 PR에서도 해당 의존성을 정확히 짚습니다.

    Args:
        message: Dependabot 커밋 메시지 전문
        name: 의존성 이름
        new_version: YAML이 명시한 변경 후 버전

    Returns:
        str | None: 변경 전 버전 문자열. 매칭되는 줄이 없으면 None.
    """
    pattern = (
        re.escape(name) + r".*?\bfrom\s+(\S+)\s+to\s+" + re.escape(new_version) + r"\b"
    )
...
def parse_dependabot_commit(message: str) -> list[dict] | None:
    ...
    updated-dependencies YAML 블록(의존성 목록과 새 버전의 정본)을 읽고,
    이전 버전만 본문에서 보강합니다. 그룹 업데이트도 동일하게 처리됩니다.
    ...
    Returns:
        list[dict] | None: [{"name": str, "before": str | None, "after": str | None}]
            ...
            제거된 의존성(새 버전이 빈 값)이나 이전 버전을 찾지 못한 경우 해당
            필드가 None이며, 호출부에서 보수적으로 차단됩니다.
    """
    ...
    for dependency in dependencies:
        ...
        after = dependency["dependency-version"]
        before = find_previous_version(message, name, after) if after else None
