...
사용법:
    python scripts/github_admin/add_ruleset.py [--dry-run] [--repo REPO_NAME]

옵션:
    --dry-run: 실제 변경 없이 어떤 리포지토리에 ruleset이 적용될지 확인
    --repo: 지정한 단일 리포지토리에만 적용 (생략 시 org 전체에 적용)
...
def load_repo_rulesets_config() -> dict[str, dict]:
    ...
    키에 glob 패턴 사용 가능 (예: "*-memory").
    expand_repo_config()로 실제 repo 이름으로 확장 필요.

    Returns:
        dict: repo 이름/패턴 → 설정 딕셔너리 매핑 (설정 파일이 없으면 빈 딕셔너리)
    """
...
def expand_repo_config(
    config: dict[str, dict], repo_names: list[str]
) -> dict[str, dict]:
    """
    config 키의 glob 패턴을 실제 repo 이름으로 확장하는 함수

    예: {"*-memory": {"skip": [...]}} + ["cmm-memory", "wam-memory"]
      → {"cmm-memory": {"skip": [...]}, "wam-memory": {"skip": [...]}}

    동일 repo에 여러 패턴이 매칭되면 ValueError를 발생시킨다.

    Args:
        config: repo별 ruleset 설정 (glob 패턴 포함 가능)
        repo_names: org에 존재하는 전체 repo 이름 목록

    Returns:
        dict: glob 패턴이 실제 repo 이름으로 확장된 설정
    """
    ...
    for pattern, conf in config.items():
        ...
        for name in matched:
            if name in source:
                raise ValueError(
                    f'repo "{name}"이 여러 config 키에 매칭됨: '
                    f'"{source[name]}", "{pattern}". 하나만 남기세요.'
                )
            source[name] = pattern
            expanded[name] = conf
    return expanded
...
def apply_repo_specific_rulesets(
    org_name: str, repo_config: dict[str, dict], dry_run: bool
) -> tuple[int, int]:
    """
    repo별 추가 ruleset을 적용하는 함수

    Args:
        org_name: Organization 이름
        repo_config: expand_repo_config()로 확장된 repo별 설정
        dry_run: dry-run 모드 여부
...
def main():
    ...
    parser.add_argument(
        "--repo",
        metavar="REPO_NAME",
        help="지정한 단일 리포지토리에만 적용 (생략 시 org 전체에 적용)",
    )
    ...
    # repo별 config 로드 및 glob 패턴 확장
    # config 검증·glob 확장은 항상 org 전체 목록 기준으로 수행한다.
    all_repo_names = [repo.name for repo in get_all_repos(org)]
    repo_config = expand_repo_config(load_repo_rulesets_config(), all_repo_names)
    ...
    # --repo 지정 시 적용 대상을 해당 단일 repo로 제한
    # (skip_repos_by_file은 위에서 org 전체 config 기준으로 이미 계산됨)
    if args.repo:
        if args.repo not in all_repo_names:
            print(f"[ERROR] repo '{args.repo}'이 org '{org_name}'에 존재하지 않습니다.")
            sys.exit(1)
        target_repo_names = [args.repo]
        repo_config = (
            {args.repo: repo_config[args.repo]} if args.repo in repo_config else {}
        )
        print(f"적용 대상: {args.repo} (단일 repo)")
    else:
        target_repo_names = all_repo_names
        print(f"적용 대상: org 전체 ({len(all_repo_names)}개 repo)")
    print("=" * 60)
    ...
    repo_success, repo_error = apply_repo_specific_rulesets(
        org_name, repo_config, args.dry_run
    )
