...
def execute_athena_query(client, query, database=ATHENA_DATABASE, output_location=ATHENA_OUTPUT_LOCATION):
...
def main():
    parser = argparse.ArgumentParser(description='Query ALB Access Logs for payload sizes')
    parser.add_argument('--path', required=True, help='Request path to analyze (e.g., /api/v1/heavy)')
    parser.add_argument('--oom-time', required=True, help='OOM time in ISO format (e.g., 2025-11-21T17:35:16)')
    parser.add_argument('--minutes-before', type=int, default=10, help='Minutes before OOM to analyze (default: 10)')
    ...
    if parsed_results:
        ...
        received_sizes = [int(r['received_bytes']) for r in parsed_results if r['received_bytes'].isdigit()]
        sent_sizes = [int(r['sent_bytes']) for r in parsed_results if r['sent_bytes'].isdigit()]
        ...
        for r in parsed_results[:5]:
            print(f"    {r['time']}: {r['sent_bytes']:>10} bytes sent, "
