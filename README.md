...
- `GOOGLE_SERVICE_ACCOUNT_JSON_OPERATE`: 운영봇 전용 서비스 계정 JSON (필수)
...
별도 계정이 필요한 이유: 기존 `GOOGLE_SERVICE_ACCOUNT_JSON`은 특정 스프레드시트에만
권한이 있는 계정이라 Drive 작업 공간에 접근할 수 없다. 폴백을 두지 않으므로 이 값이
없으면 기동 중 즉시 실패한다 — 권한 없는 계정으로 붙어 원인 모를 404를 내는 것보다 낫다.
기존 `get_worksheet_values`는 원래 계정을 그대로 쓰므로
