...
- 채널의 `CLAUDE.md` 캔버스를 매 요청마다 읽어 시스템 프롬프트에 넣는다
- **캔버스에 적힌 Drive 폴더가 그 채널의 작업 공간이 된다.** 검색·읽기·쓰기가 모두
  그 폴더의 하위 트리로 제한된다

작업 공간 제한은 프롬프트 지시가 아니라 코드에서 건다. 검색 쿼리에는 하위 폴더 조건이
자동으로 AND로 붙고, 읽기·쓰기는 대상 파일의 상위 폴더가 범위 안인지 확인한 뒤에만
진행한다. 모델이 범위를 벗어날 방법이 없다.
...
채널마다 캔버스에 다른 폴더를 적으면 채널마다 다른 작업 공간이 된다.
캔버스에 넣을 초기 내용은 `docs/channel_canvas_template.md` 참고.

캔버스 이름은 `CLAUDE.md`다(대소문자 무관). 채널에 회의록 같은 다른 캔버스가 있어도
이것만 읽는다. 이름이 맞는 것이 없으면 캔버스가 하나뿐일 때만 그것을 쓰고, 여러 개면
읽지 않는다 — 엉뚱한 캔버스를 채널 기준으로 삼는 것보다 안 읽는 편이 안전하다.
...
캔버스 본문만 돌려주는 전용 메서드는 없다(`canvases.sections.lookup`은 섹션 ID만 준다).
대신 `files.list?types=canvas`로 채널의 캔버스를 찾고, 파일 내용 조회의 표준 경로인
`url_private_download` + `files:read`로 본문을 내려받는다. 두 단계 모두 공식 문서에 있다.
다만 내려받은 본문의 포맷은 명시되어 있지 않아 실제 워크스페이스에서 확인이 필요하다.
