...
**캠페인 열의 제목은 고치지 마세요.** 열을 이름으로 찾으므로, 제목을 바꾸면
그 캠페인은 자기 기록을 못 찾고 새 열을 만들어 전원에게 다시 보냅니다.
반대로 정말 재발송하고 싶으면 그 칸을 지우거나 다른 `--campaign` 을 씁니다.

예약 발송은 접수 시각이 아니라 `예약 2026-08-13 09:00` 처럼 나갈 시각을 적습니다.
