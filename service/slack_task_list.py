...
행이 있느냐로만 정해집니다. 지식베이스와 같은 Postgres를 쓰지만 수집 대상 표와
섞지 않으려고 표를 따로 둡니다.
...
def reject_task_list_channel(info: dict[str, Any]) -> str | None:
...
def save_channel_task_list(
    ...
    유니크 위반으로 드러나는 편이 이전 리스트 링크가 말없이 사라지는 것보다 낫습니다.
...
def delete_channel_task_list(channel_id: str) -> str | None:
    ...
    슬랙 리스트 자체와 채널 북마크는 그대로 둡니다. 쌓인 작업을 지우지 않고
    작업이 가는 곳만 노션으로 되돌립니다.
...
async def list_all_items(client: AsyncWebClient, list_id: str) -> list[dict]:
    """리스트의 항목을 전부 읽습니다.
...
async def create_channel_task_list(
    ...
    표에 먼저 넣고 공유와 북마크를 뒤에 합니다. 뒤쪽이 실패해도 만들어진
    ...
    created = await client.slackLists_create(
        name=build_list_name(channel_name), todo_mode=True
    )
