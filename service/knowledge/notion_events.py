...
주기 실행(scripts/sync_knowledge_notion.py)과 역할이 갈립니다. 웹훅은 바뀐
페이지 하나를 곧바로 반영하고, 주기 실행은 권한 안 전체를 열거해 정합성을
맞춥니다. 웹훅만으로는 부족한 이유가 셋입니다.

- 전달이 at-most-once다. 8회 재시도하고 약 24시간 뒤 포기하며 그걸로 끝이다
- 같은 페이지에 몰린 이벤트는 집계되어, 상태가 원래대로 돌아오면 아예 안 온다
- 통합 권한을 내린 것을 알려주는 이벤트가 없다. 수집 범위가 곧 권한이므로
  권한이 빠진 것은 전량 열거로만 알 수 있다
...
def resolve_root(
    ...
    API로 타고 올라가는데, 그게 여기서 필요한 동작입니다. 주기 실행은 search
    결과를 통째로 넘겨 대부분의 조상을 이미 알고 들어갑니다.
