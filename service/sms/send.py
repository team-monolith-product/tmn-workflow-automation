...
②의 실패가 어느 쪽인지는 transport.PpurioError 가 정합니다 — 그 예외의
docstring 이 정본입니다.
"""
...
class Plan(NamedTuple):
    ...
    problems: list[str]  # 비어 있지 않으면 보낼 수 없다
    template: str  # 치환 태그가 남아 있는 원문
    rows: list[dict[str, Any]]  # 번호 정규화 + 중복 접기까지 끝난 목록
    folded: int  # 접힌 중복 번호 수
    message_type: str | None  # SMS · LMS · MMS
    send_time: str | None  # 벤더 sendTime. 즉시 발송이면 None
    send_at: datetime.datetime | None  # KST 벽시계로 눕힌 예약 시각
...
def _plan(
    ...
    판정과 산출을 갈라 두면 사본이 생깁니다. 그러면 나중에 규칙 하나가 한쪽에만
    들어가는 순간, 사람에게 보여준 것(check)·실제로 막히는 것(send_campaign)·
    미리보기(preview)가 서로 다른 말을 합니다.
    ...
    # 명단에 같은 사람이 두 번 들어오는 일이 실제로 있다. 접지 않으면
    # ledger.claim 이 거절한다 — 한 사람에게 문안 두 벌을 보내라는 뜻이라
    # 어느 쪽이 맞는지 코드가 정할 수 없기 때문이다.
    problems: list[str] = []
...
def send_campaign(
    ...
    **vendor: Any,
    ...
    Args:
        spreadsheet_id: 기록을 적을 참가자 스프레드시트
        campaign: 발송 건 식별자. 명단의 열 제목이 된다
        rows: to·name·var1~var8 을 담은 수신자 목록
        template_name: templates/sms/{name}.txt (content 와 택일)
        content: 즉석 문안 본문 (template_name 과 택일)
    ...
    claimed = ledger.claim(ws, campaign, plan.rows)
    code = message_key = None
    message_type = plan.message_type

    if claimed.won:
        sent_to = [entry["to"] for entry in claimed.won]
        # 실제로 보낼 사람만으로 다시 판정한다. 전원 기준으로 정하면 이미
        # 받아서 빠진 사람의 긴 치환값 때문에 캠페인 전체가 LMS 로 올라가
        # 요금을 더 낸다. won 은 plan.rows 의 부분집합이라 여기서 안 터진다.
        message_type = templates.decide_message_type(plan.template, claimed.won)
        payload = {
            "messageType": message_type,
            "content": plan.template,
            "targetCount": len(claimed.won),
            "targets": templates.build_targets(claimed.won),
            "refKey": campaign,
            **vendor,
        }
        if message_type != "SMS":
        ...
        stamp = (
            f"예약 {plan.send_at.strftime('%Y-%m-%d %H:%M')}"
            if plan.send_at is not None
    ...
    return {
        ...
        "skipped": len(claimed.done),
        "blocked": [entry["to"] for entry in claimed.blocked],
        "missing": [entry["to"] for entry in claimed.missing],
        "sent": len(claimed.won),
        "sent_to": [entry["to"] for entry in claimed.won],
        "code": code,
